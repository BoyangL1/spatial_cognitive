...
def update_neuron_activity(GN,r, r_r, r_l, r_d, r_u,r_masks,r_fft_plan, r_ifft_plan, vx, vy, r_field, itter, singleneuronrec, time_ind, sna_eachlayer, row_record, col_record, w_r, w_l, w_u, w_d, a):
    ...
    # calculate fields
    [r_l, r_field_l] = RN.calculate_field(r, r_l, rwu_l, rwu_r, rwu_d, rwu_u, r_masks[0, :, :], a, 1.0-GN.vgain*vx, GN.h, GN.n, GN.npad, 0)
    [r_r, r_field_r] = RN.calculate_field(r, r_r, rwu_l, rwu_r, rwu_d, rwu_u, r_masks[1, :, :], a, 1.0+GN.vgain*vx, GN.h, GN.n, GN.npad, 0)
    [r_u, r_field_u] = RN.calculate_field(r, r_u, rwu_l, rwu_r, rwu_d, rwu_u, r_masks[2, :, :], a, 1.0+GN.vgain*vy, GN.h, GN.n, GN.npad, 0)
    [r_d, r_field_d] = RN.calculate_field(r, r_d, rwu_l, rwu_r, rwu_d, rwu_u, r_masks[3, :, :], a, 1.0-GN.vgain*vy, GN.h, GN.n, GN.npad, 0)
...
def update_neuron_activity_with_place(GN,r, r_r, r_l, r_d, r_u, r_masks,r_fft_plan, r_ifft_plan, vx, vy, r_field, itter, singleneuronrec, time_ind, sna_eachlayer, row_record, col_record, curr_place_activity, w_r, w_l, w_u, w_d, a, w_pg, n_place_cells):
    """
    ...
    curr_place_activity = curr_place_activity.reshape((n_place_cells, 1, 1, 1))
    p = np.sum((curr_place_activity*w_pg), 0) # [h,n_grid_cells,n_grid_cells]

    # calculate fields
    [r_l, r_field_l] = RN.calculate_field(
        r, r_l, rwu_l, rwu_r, rwu_d, rwu_u, r_masks[0, :, :], a, 1.0-GN.vgain*vx, GN.h, GN.n, GN.npad, p)
    [r_r, r_field_r] = RN.calculate_field(
        r, r_r, rwu_l, rwu_r, rwu_d, rwu_u, r_masks[1, :, :], a, 1.0+GN.vgain*vx, GN.h, GN.n, GN.npad, p)
    [r_u, r_field_u] = RN.calculate_field(
        r, r_u, rwu_l, rwu_r, rwu_d, rwu_u, r_masks[2, :, :], a, 1.0+GN.vgain*vy, GN.h, GN.n, GN.npad, p)
    [r_d, r_field_d] = RN.calculate_field(
        r, r_d, rwu_l, rwu_r, rwu_d, rwu_u, r_masks[3, :, :], a, 1.0-GN.vgain*vy, GN.h, GN.n, GN.npad, p)
    ...
    # update fields and weights
    r = RN.update_activity_spike(r, r_field, GN.h, GN.n, GN.dt, GN.tau)
    w_pg = RN.update_weights(n_place_cells, curr_place_activity, r, w_pg)
...
def flow_full_model(GN, anchor_list ,x, y, vx, vy, time_ind, a, r, r_r, r_l, r_d, r_u, r_masks,singleneuronrec,place_cell_spiking, place_activity, w_pg, place_cells, n_place_cells, w_r, w_l, w_u, w_d):
    """ 
    ...
    for itter in tqdm(range(1, time_ind, 1)):
        ...
        [r, r_field, r_l, r_u, r_d, r_r, sna_eachlayer, w_pg] = update_neuron_activity_with_place(
            GN, r, r_r, r_l, r_d, r_u, r_masks, r_fft_plan, r_ifft_plan, vx1, vy1, r_field, itter, singleneuronrec, time_ind, sna_eachlayer, row_record, col_record, curr_place_activity, w_r, w_l, w_u, w_d, a, w_pg, n_place_cells)
        # save anchor points and their attached grid cells network
        if (x[itter],y[itter]) in anchor_list:
            place_grid_dic[(x[itter], y[itter])] = r

    return r, r_field, r_r, r_l, r_d, r_u, sna_eachlayer,w_pg, place_grid_dic