...
# normalization constant for place cells with sigma = 5.0
...
def check_centers(p_cells, dist_thresh, rand_x_center, rand_y_center):
    ...
    for i in range(0, len(p_cells)):
        ...
        curr_dist = np.sqrt((rand_x_center - curr_x_center)
                            ** 2 + (rand_y_center - curr_y_center)**2)
...
def create_place_cells(n_place_cells, sigma, x_dim, y_dim, dist_thresh, map_b=False):
    ...
    for i in range(0, n_place_cells):
        ...
        p_cells.append(
            PlaceCell(rand_x_center, rand_y_center, sigma, x_dim, y_dim))
    return p_cells
...
def plot_centers(place_cells, dist_thresh):
    ...
    for i in range(0, len(place_cells)):
        ...
        for j in range(i+1, n_place_cells):
            ...
            curr_dist = np.sqrt((curr_x_to_compare - curr_x_center)
                                ** 2 + (curr_y_to_compare - curr_y_center)**2)
            if curr_dist <= dist_thresh:
                print('Place centers too close: ', (curr_x_center, curr_y_center),
                      (curr_x_to_compare, curr_y_to_compare), int(curr_dist))
    ...
    for i in range(0, len(place_cells)):
        ...
        for j in range(0, x_dim):
            for k in range(0, y_dim):
                curr_place_val = (1/(2*np.pi*sigma**2)) * np.exp(-(
                    (j - curr_x_center)**2 + (k - curr_y_center)**2) / (2*sigma**2))
...
def evaluate_spiking(place_cell_spiking, place_activity, place_cells, x, y, time_idx):
    ...
    for i in range(0, len(place_cells)):
        ...
        place_cell_spiking[i, time_idx], place_activity[i,
                                                        time_idx] = curr_place_cell.evaluate_spiking(x[time_idx], y[time_idx])
